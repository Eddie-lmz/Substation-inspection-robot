...
def template_matching(img,path):
    ...
    if(val_list[index_max_val]<0.90 and val_list[index_max_val]>0.60):
        # val_listfinal.append(val_list[index_max_val])
        # loc_listfinal.append(loc_list[index_max_val])
        if(path == "clock_2/number00.jpg"):
            number = 0
        else:
            number = 10
...
def Pointer_recognition(img,center,orign_img):
    appearance = img
    path1 = "clock_2/number00.jpg"
    path2 = "clock_2/number22.jpg"
...
if __name__ == '__main__':
    img = cv2.imread("clock_2/type2.jpg")
    Pointer_recognition(img, [209,203])